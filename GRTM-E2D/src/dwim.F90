...
module dwimMod
  ...
  use paraMod, only: tRec, faLim, kfCri, kLim, dt => dtRec, dkLim, xs, zs, &
    & xr, zr, svInty, swType, swTime, swFreq, srTime, isForce, lSrc, lRec, z, &
    & beta, alpha
  use grtcMod
  implicit none
  private
  ...
  integer, parameter :: nIntg = 8, npt = 10
  ...
  complex(kind = MK), allocatable :: iUx(:), iUy(:), iUz(:)
  complex(kind = MK), allocatable :: iTxz(:), iTyz(:), iTzz(:)
  complex(kind = MK), allocatable :: iTxx(:), iTxy(:)
  real(kind = MK), allocatable :: ut(:)
  real(kind = MK), allocatable, public :: t(:)
  real(kind = MK), allocatable, public :: ux(:), uy(:), uz(:)
  real(kind = MK), allocatable, public :: txz(:), tyz(:), tzz(:)
  real(kind = MK), allocatable, public :: txx(:), txy(:)
  ...
  contains
    ...
    subroutine dwimInitialize()
      ...
      allocate(iUy (nt)); iUy  = (0.0_MK, 0.0_MK)
      allocate(iUz (nt)); iUz  = (0.0_MK, 0.0_MK)
      allocate(iTxz(nt)); iTxz = (0.0_MK, 0.0_MK)
      allocate(iTyz(nt)); iTyz = (0.0_MK, 0.0_MK)
      allocate(iTzz(nt)); iTzz = (0.0_MK, 0.0_MK)
      allocate(iTxx(nt)); iTxx = (0.0_MK, 0.0_MK)
      allocate(iTxy(nt)); iTxy = (0.0_MK, 0.0_MK)
    ...
    subroutine dwimRun()
      ...
      do i = fi1, fi2
        ...
        iUy (i) = - uIntg(2) * (0.0_MK, 1.0_MK)
        iUz (i) = - uIntg(3)
        iTxz(i) =   uIntg(4) * (0.0_MK, 1.0_MK)
        iTyz(i) = - uIntg(5) * (0.0_MK, 1.0_MK)
        iTzz(i) = - uIntg(6)
        iTxx(i) = - uIntg(7)
        iTxy(i) =   uIntg(8)
      end do
      ...
      call FFTW_(execute_dft_c2r) (p, iUy , ut); uy  = ut(1:ntRec) * df * exp(feps * t)
      call FFTW_(execute_dft_c2r) (p, iUz , ut); uz  = ut(1:ntRec) * df * exp(feps * t)
      call FFTW_(execute_dft_c2r) (p, iTxz, ut); txz = ut(1:ntRec) * df * exp(feps * t)
      call FFTW_(execute_dft_c2r) (p, iTyz, ut); tyz = ut(1:ntRec) * df * exp(feps * t)
      call FFTW_(execute_dft_c2r) (p, iTzz, ut); tzz = ut(1:ntRec) * df * exp(feps * t)
      call FFTW_(execute_dft_c2r) (p, iTxx, ut); txx = ut(1:ntRec) * df * exp(feps * t)
      call FFTW_(execute_dft_c2r) (p, iTxy, ut); txy = ut(1:ntRec) * df * exp(feps * t)
      ...
      iUy (nt:nt/2 + 2:-1) = conjg( iUy (2:nt/2) )
      iUz (nt:nt/2 + 2:-1) = conjg( iUz (2:nt/2) )
      iTxz(nt:nt/2 + 2:-1) = conjg( iTxz(2:nt/2) )
      iTyz(nt:nt/2 + 2:-1) = conjg( iTyz(2:nt/2) )
      iTzz(nt:nt/2 + 2:-1) = conjg( iTzz(2:nt/2) )
      iTxx(nt:nt/2 + 2:-1) = conjg( iTxx(2:nt/2) )
      iTxy(nt:nt/2 + 2:-1) = conjg( iTxy(2:nt/2) )

      ut = real( fft(nt, iUx , -1) ); ux  = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iUy , -1) ); uy  = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iUz , -1) ); uz  = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iTxz, -1) ); txz = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iTyz, -1) ); tyz = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iTzz, -1) ); tzz = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iTxx, -1) ); txx = ut(1:ntRec) * df * exp(feps * t)
      ut = real( fft(nt, iTxy, -1) ); txy = ut(1:ntRec) * df * exp(feps * t)
    ...
    subroutine dwimFinalize()
      ...
      if(allocated(iUy )) deallocate(iUy )
      if(allocated(iUz )) deallocate(iUz )
      if(allocated(iTxz)) deallocate(iTxz)
      if(allocated(iTyz)) deallocate(iTyz)
      if(allocated(iTzz)) deallocate(iTzz)
      if(allocated(iTxx)) deallocate(iTxx)
      if(allocated(iTxy)) deallocate(iTxy)
    ...
    function integrand(k, omg, Sw) result(intg)
      ...
      if(isForce) then
        call grtcKernelForceSH(k, Sw)
      else
        call grtcKernelMomentSH(k, Sw)
      end if
#endif
#ifdef PS
      ...
      if(isForce) then
        call grtcKernelForcePS(k, omg, Sw)
      else
        call grtcKernelMomentPS(k, omg, Sw)
      end if
#endif
      call grtcExtraStress(k)

      !ref.: eqs. (4-4a, b and c)
      eikx = exp( (0.0_MK, 1.0_MK) * k * xr )
      intg(1) = DPS(1) * eikx ! for Ux
      intg(2) = WSH    * eikx ! for Uy
      intg(3) = DPS(2) * eikx ! for Uz
      intg(4) = OPS(1) * eikx ! for Txz
      intg(5) = TSH    * eikx ! for Tyz
      intg(6) = OPS(2) * eikx ! for Tzz
      intg(7) = tP     * eikx ! for Txx
      intg(8) = tV     * eikx ! for Txy
