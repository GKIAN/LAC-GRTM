...
module paraMod
  ...
  contains
    ...
    subroutine paraPrintHelp()
      ...
      write(*, '(2X, A)') '             calculate 3-D elastic waveform ' &
      ...
      open(newunit = fileID, file = inputFile, status = 'old')
        ...
        select case(trim(adjustl(srcType)))
          ...
          case('moment')
            ...
            call commGetConf(fileID, toBack, 'source_moment_tensor', 2, mxyz(2, 2))
            call commGetConf(fileID, toBack, 'source_moment_tensor', 3, mxyz(3, 3))
            call commGetConf(fileID, toBack, 'source_moment_tensor', 4, mxyz(1, 2))
            call commGetConf(fileID, toBack, 'source_moment_tensor', 5, mxyz(2, 3))
            call commGetConf(fileID, toBack, 'source_moment_tensor', 6, mxyz(1, 3))
            call refineMoment()
          case('angle')
